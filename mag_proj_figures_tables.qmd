...
variable_names <- list(
  ...
  'mean_cells_mL'= bquote("mean cells" ~mL^-1),
  'Nitrate_Average'=bquote("Nitrate Average (µM)"),
  'Phosphate_Average'=bquote('Phosphate Average (µM)')
)
...
lmo_id_date <- read_tsv("../data/lmo_id_date.tsv.gz") %>%
 ...
 plot2 <- p2 %>%
  ggplot(mapping = aes(x = dates, y = tpm, fill = fct_relevel(new_phylum, "Other", after = Inf))) +
  geom_area() +
  #scale_x_date(breaks =  unique(c(
...
obj <- BACL_taxonomy %>%
 ...
 TEXT_SIZE = 10
pheatmap(t(top50_df[,rev(bacl_order)]), cluster_cols = F, cluster_rows = F, scale = "row",  border_color = NA, fontsize_row = as.numeric(TEXT_SIZE), fontsize_col = as.numeric(TEXT_SIZE), annotation_col = rev(ann_col), annotation_row = ann_row,  annotation_names_row = F, annotation_names_col = F, annotation_colors = annCol, fontsize = as.numeric(TEXT_SIZE), cellheight = 9, cellwidth = 7.2, filename = "../results/figure_3_pheatmap.png", width = 6.29921, height = 8)
```
...
png("../results/cumulative_fraction_curve.png") 
  plot(cumsum(sort(rank_df_tpm$fraction, decreasing = T)) , main = "cumulative percentage curve", 
...
pheatmap(magmat[cluster$order,],  cluster_rows = F, cluster_cols = F, scale = "row", main = paste0(bacl), filename = paste0("../results/pheatmap_agnes/",bacl,".png"), width = (5*0.393701), height = (8*0.393701), show_rownames = F, fontsize = 7)
}
```
...
pheatmap(bin236_clust, cluster_cols = F, cluster_rows = T, clustering_distance_rows = "correlation", scale = "row", show_rownames = F, annotation_row = annotation_rows, main = paste0(bacl), width = (9*0.393701), height = (6*0.393701), fontsize = 7, filename = paste0("../results/cluster_graphs/",bacl,"cluster",".pdf"),  treeheight_row = 1, annotation_legend = FALSE)
...
ggplot(mapping = aes(x = dates, y = tpm, group = BACL)) +
    geom_line() +
    ...
    ylab("") +
    ggsave("../results/all_mag_community_dynamics.png")
```
